...
/* ══════════════════════════════════════════════
   ZONA LANDING PAGE — Dynamic SEO landing
   Route: /asesoria-legal-para-agencias/:slug
   Data: Wix CMS collection "EstadosPaginas"
   Each landing is 100% driven by CMS fields:
   title, zona, excerpt, seoTitle,
   metaDescription, whatsappUrl, slug
   ══════════════════════════════════════════════ */
...
export default function ZonaLandingPage() {
  ...
  /* WhatsApp URL: prefer CMS value, fallback to siteConfig */
  const whatsappUrl = page.whatsappUrl || siteConfig.whatsapp;
  const zona = page.zona || page.title;

  return (
    <>
      {/* Dynamic <title> and <meta description> from CMS */}
      ...
      <div className="zona-landing">
        ...
        <section className="zona-hero">
          <div className="container zona-hero__inner">
            ...
            <span className="zona-label">Asesoría Legal · {zona}</span>

            <h1 className="zona-title">{page.title}</h1>

            {page.excerpt && (
              <p className="zona-excerpt">{page.excerpt}</p>
            )}
        ...
        {/* ── Shared components — not hardcoded ── */}
        <Services />
        <Testimonials />
        <CTABanner
          title={`¿Tu empresa opera en ${zona}?`}
